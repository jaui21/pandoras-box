...
if ( !isset($_GET['subscription_id']) ) {
	...
	if ( !$subscription ) {
		...
		if ( $subscription->braintree_subscription_id ) {
			$subscription_bt = Braintree_Subscription::find($subscription->braintree_subscription_id);
			if ( $subscription->status == 'Active' && $subscription_bt->status !== 'Active' ) {
			...
			<?php if ( !isset($error) ) : ?>
				...
				<div class="form-horizontal">
					...
					<div class="form-group">
						...
						<div class="col-md-10 form-control-static">
							<?php echo $provider == 'braintree' ? $subscription->braintree_subscription_id : ( $provider == 'paypal' ?  $subscription->paypal_subscription_id : ''); ?>
						</div>
					</div>
					<div class="form-group">
					...
					<?php if ( $subscription->status == 'Active' ) : ?>
						<div class="form-group">
							<div class="col-sm-10 col-sm-offset-2">
								<?php if ( $provider == 'braintree' ) : ?>
									<a href="<?php echo url('process.php?action=cancel_subscription&subscription_id=' . $subscription->id); ?>" class="btn btn-default confirm-click" data-text="Yes, cancel it!"><fa class="fa fa-times colordanger"></fa> Cancel Subscription</a>
