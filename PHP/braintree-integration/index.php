...
if ( empty($config['name']) || empty($config['email']) ) {
	$error = 'You need to enter your name and email in the admin settings area before you can accept payments.';
}
if ( $config['enable_paypal'] && empty($config['paypal_email']) ) {
	$error = 'You need to enter your PayPal email address in the admin settings area before you can accept PayPal payments.';
}
if ( empty($config['braintree_merchant_id']) || empty($config['braintree_public_key']) || empty($config['braintree_private_key']) ) {
	$error = 'You need to enter your Braintree details in the admin settings area before you can accept credit card payments.';
}
if ( $config['enable_subscriptions'] && !$config['enable_paypal'] && empty($config['braintree_plan_id']) ) {
	$error = 'You need to enter your Braintree subscription plan ID in the admin settings area before you can accept recurring payments.';
}
if ( isset($error) ) {
	$allow_submit = false;
}
    ...
    <body class="terminal-body">
		...
		<div class="container terminal-wrapper">
	    	...
	    	<form action="<?php echo url('process.php'); ?>" method="post" class="validate form-horizontal <?php echo !$allow_submit ? 'disabled' : ''; ?>" id="order_form">
	    		...
	    		<?php endif; ?>
				<input type="hidden" class="client-token" value="<?php echo !isset($error) ? Braintree_ClientToken::generate() : ''; ?>">
					...
					<div class="col-md-6">
						...
						<div class="creditcard-content">
							...
							<div class="form-group">
								...
								<div class="col-md-9">
									<div class="input-group">
										<input type="text" data-braintree-name="cardholder_name" name="cardholder_name" class="form-control" placeholder="Name on Card" value="" data-rule-required="true">
							...
							<div class="form-group">
								...
								<div class="col-md-9">
									<div class="input-group">
										<input type="text" data-braintree-name="number" class="form-control card-number" placeholder="Card Number" value="" data-rule-required="true" data-rule-creditcard="true">
										<span class="input-group-addon"><i class="fa fa-lock"></i></span>
									</div> 
							...
							<div class="form-group">
								<label class="control-label col-md-3"><span class="colordanger">*</span>Expiration/CVV</label>
								<div class="col-md-9">
									<div class="row">
										<div class="col-md-4 col-xs-4 pr5">
											<select data-braintree-name="expiration_month" class="form-control" data-rule-required="true">
							                    <?php for ( $i = 1; $i <= 12; $i++ ) : ?>
										...
										<div class="col-md-4 col-xs-4 pl5 pr5">
											<select data-braintree-name="expiration_year" class="form-control" data-rule-required="true">
										...
										<div class="col-md-4 col-xs-4 pl5">
											<div class="input-group">
												<input type="text" data-braintree-name="cvv" name="cvv" class="form-control" placeholder="CVV" value="" data-rule-required="true">
												<span class="input-group-addon"><i class="fa fa-lock"></i></span>
											</div> 
										</div>
									</div>
	    	...
	    	<?php if ( $config['enable_paypal'] ) : ?>
				...
				<?php if ( $config['enable_subscriptions'] == 'braintree_and_paypal' || $config['enable_subscriptions'] == 'paypal_only' ) : ?>
